...
const buildJoinsAndFilter = (qb, model, filters) => {
  ...
  const generateNestedJoins = (field, tree) => {
    ...
    if (parts.length === 0) {
      if (['manyToMany', 'manyWay'].includes(assoc.nature)) {
        // Constraint is stored in related table: apply LEFT JOIN, then force value
        parts = [assocModel.primaryKey];
      } else {
        // Constraint is stored in main table: filter on requested field
        return `${tree.alias}.${key}`;
      }
    }
...
const buildWhereClause = ({ qb, field, operator, value }) => {
  ...
  switch (operator) {
    ...
    case 'search':
      return qb.whereRaw(`MATCH (${fieldLowerFn(qb)}) AGAINST (? IN BOOLEAN MODE)`, [field, `${value}`]);
    case 'null': {
