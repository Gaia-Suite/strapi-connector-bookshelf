  ...
  _.forEach(
    ...
    (connection, name) => {
      ...
      switch (options.client) {
        // mysql2 shares this arm rather than getting its own: its typeCast wrapper passes the same
        // shape - `field.type` as a string name ('DECIMAL', 'NEWDECIMAL', 'TINY') and `field.length`
        // as the declared column width - so the body below is correct verbatim under both drivers.
        // Falling through is what keeps DECIMAL a Number and TINYINT(1) a boolean after the swap;
        // without it every price and every todo_* flag silently changes type.
        case 'mysql':
        case 'mysql2':
