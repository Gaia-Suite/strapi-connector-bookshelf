  ...
  const updateModel = async model => {
    ...
    try {
      ...
      loadedModel.initialize = function() {
        ...
        // MySQL is not in bookshelf's supportsReturning() list (bookshelf/lib/sync.js), so every
        // save() ends in refresh() (model.js, just below the autoRefresh check) - a SELECT that the
        // `fetching` hook above then turns into the full default populate, because refresh() passes
        // no withRelated. On `product` that is 13 extra queries per save, and nothing reads the
        // result: `this.id` is assigned from the insertId before the refresh, timestamps are set in
        // JS, and the entity returned by create/update comes from a separate read (relations.update's
        // final fetch, or queries.js findOne). Left overridable - a caller passing autoRefresh keeps it.
        this.on('saving', (instance, attrs, options) => {
          if (options.autoRefresh === undefined) options.autoRefresh = false;
        });
