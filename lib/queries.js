...
module.exports = function createQueryBuilder({ model, strapi }) {
  ...
  // `oneWay` and `manyToOne` both compile to a bookshelf `belongsTo` whose foreign key is a column of
  // this very table (mount-models' `case 'belongsTo'`), and relations.js writes them with a bare
  // `_.set` and no relation-side statement. So they can travel in the INSERT/UPDATE instead of being
  // stripped out, written as the column default, and then patched back in by a second statement from
  // updateRelations. `oneToOne` is deliberately not foldable: which side holds the key varies, and its
  // branch also nulls the previous owner.
  const FOLDABLE_NATURES = ['oneWay', 'manyToOne'];
  const foldableAssociations = model.associations.filter(ast => FOLDABLE_NATURES.includes(ast.nature));
  const foldableKeys = foldableAssociations.map(ast => ast.alias);
  const deferredRelationKeys = _.difference(assocKeys, foldableKeys);

  // Returns an object with relation keys only to create relations in DB
  // `_.pickBy` rather than `_.pick`: a key present with an undefined value is dropped, matching what
  // relations.update does with it anyway (removeUndefinedKeys). Without this, `{ tags: undefined }`
  // counts as a relation for needsTransaction below and opens a transaction that writes nothing.
  const pickRelations = attributes => {
    return _.pickBy(
      attributes,
      (value, key) => deferredRelationKeys.includes(key) && value !== undefined
    );
  };

  // The foldable relations, as the columns they actually are. Normalised exactly as relations.js does
  // it, so `5`, `'5'`, `{ id: 5 }` and `null` all behave as before.
  const foldForeignKeys = attributes => {
    return foldableAssociations.reduce((acc, ast) => {
      if (!_.has(attributes, ast.alias) || attributes[ast.alias] === undefined) return acc;

      const assocModel = strapi.db.getModelByAssoc(ast);
      const columnName = _.get(model.attributes[ast.alias], 'columnName', ast.alias);

      acc[columnName] = _.get(attributes[ast.alias], assocModel.primaryKey, attributes[ast.alias]);
      return acc;
    }, {});
  };
  ...
  // A write needs a transaction only when it may issue more than one *write* statement, which is
  // exactly when the payload carries components or a deferred relation. Phrased as a property of the
  // payload rather than an exemption per operation: with the foreign keys folded into the INSERT
  // above a create writes once, but if it ever went back to INSERT-then-UPDATE while skipping the
  // transaction, the row would be briefly visible with a NULL foreign key to the crons that poll
  // these very tables by todo_* flag. (The read-back is a statement too, but it reads.)
  //
  // Note the transaction never covered as much as it appears to: lifecycle hooks run outside it
  // (strapi-database wraps the whole connector call), so a model's before/after work was never atomic
  // with the row. What it does cost is two round-trips and a pooled connection pinned for the whole
  // callback.
  const needsTransaction = ({ relations, attributes }) =>
    componentKeys.some(key => _.has(attributes, key)) || Object.keys(relations).length > 0;

  const wrapTransaction = (fn, { transacting, needsTransaction: needs = true } = {}) => {
    const db = strapi.connections[model.connection];

    if (transacting) return fn(transacting);
    if (!needs) return fn(undefined);
  ...
  async function create(attributes, { transacting } = {}) {
    ...
    const data = { ...selectAttributes(attributes), ...foldForeignKeys(attributes) };
    ...
    return wrapTransaction(runCreate, {
      transacting,
      needsTransaction: needsTransaction({ relations, attributes }),
    });
  }

  async function update(params, attributes, { transacting } = {}) {
    // withRelated: false is populate.js's opt-out, and this read has no use for relations - `entry`
    // serves only to raise the 404, to carry the id, and as the instance to save onto. Left as a full
    // column read on purpose: bookshelf gates `updated_at` on hasChanged(), which diffs against the
    // fetched row, so narrowing this to `columns: [primaryKey]` would start bumping updated_at on
    // no-op updates - and updated_at drives the cron windows.
    const entry = await model.where(params).fetch({ transacting, withRelated: false });
    ...
    return wrapTransaction(runUpdate, {
      transacting,
      needsTransaction: needsTransaction({ relations, attributes }),
    });
  }
      ...
      return strapi
        ...
        .then(component => {
          return joinModel.forge().save(
            ...
            // no `initialize` on the join model, so the autoRefresh default does not reach it
          ...
          .then(component => {
            return joinModel
              ...
              .save(
                ...
                { transacting, patch: true, require: false, autoRefresh: false }
              );
          });
      }
      // create
...
const buildSearchQuery = ({ model, params }) => qb => {
  ...
  switch (model.client) {
    ...
    case 'mysql2':
      ...
      break;
    default:
      // No arm means no clause is added at all, so `_q` would quietly return the whole table
      // instead of erroring - a wrong answer, not a failure. Refuse instead.
      throw new Error(
        `buildSearchQuery: unsupported client \`${model.client}\` - _q search would return unfiltered rows`
      );
  }
};
